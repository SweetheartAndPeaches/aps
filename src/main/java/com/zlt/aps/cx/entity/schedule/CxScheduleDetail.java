 ...
 * 排程明细表（车次级别）
 *
 * <p>每条记录代表一个车次的排产明细，8个班次排量分别映射到 CLASS1~8 字段。
...
@ApiModel(value = "排程明细对象", description = "排程明细表（车次级别）")
public class CxScheduleDetail extends BaseEntity {
    ...
    @ApiModelProperty(value = "本车次容量")
    @TableField("TRIP_CAPACITY")
    private Integer tripCapacity;

    @ApiModelProperty(value = "本车次实际完成量")
    ...
    @ApiModelProperty(value = "全局排序号")
    @TableField("SEQUENCE")
    private Integer sequence;

    @ApiModelProperty(value = "班次内排序号")
